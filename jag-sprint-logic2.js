export function attachSprintHandler(jaguar, updatePosition, manageInterval) {
    ...
    document.addEventListener('keydown', e => {
        ...
        if (e.shiftKey && arrowKeyPressed && !isSprinting) {
            startSprint(arrowKeyPressed === 'ArrowRight' ? 'right' : 'left');
        }
    });

    document.addEventListener('keyup', e => {
        if (e.key === 'Shift' || e.key === arrowKeyPressed) {
            stopSprint();
            arrowKeyPressed = null;  // Reset the arrow key state
        }
    });
}
